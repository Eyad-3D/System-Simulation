...
function TopologyCanvasInner() {
  ...
  const edges: Edge[] = useMemo(() => {
    ...
    for (const c of system.connections) {
      ...
      const filterKey = kind === "mechanical" ? "mechanical" : kind === "signal" ? "signal" : "electrical";
    ...
    if (visibleKinds.signal) {
      for (const d of project.dataBusConnections) {
        if (!elementIds.has(d.element1Id) || !elementIds.has(d.element2Id)) continue;
        // draw from the output side so the edge leaves a source handle
        const el1 = system.elements.find((e) => e.id === d.element1Id);
        const dir1 = el1
          ? libraryById[el1.componentDefId]?.ports.find((p) => p.id === d.port1Id)?.direction
          : undefined;
        const [src, srcPort, tgt, tgtPort] =
          dir1 === "input"
            ? [d.element2Id, d.port2Id, d.element1Id, d.port1Id]
            : [d.element1Id, d.port1Id, d.element2Id, d.port2Id];
        out.push({
          id: d.id,
          source: src,
          ...
          type: "smoothstep",
          style: { stroke: KIND_COLOR.signal, strokeDasharray: "6 3" },
          selected: selectedEdges.has(d.id),
        });
      }
    }
    return out;
  ...
  const isValidConnection: IsValidConnection = useCallback(
    (conn) => {
      ...
      if (a.kind === "signal" || b.kind === "signal") {
        return a.kind === "signal" && b.kind === "signal" && a.direction !== b.direction;
      }
    ...
    <div className="flex h-full flex-col">
      <div className="ss-panel-toolbar justify-between">
        <div className="flex min-w-0 items-center gap-0.5 text-[12px]">
          {breadcrumb.map((sys, i) => (
            <span key={sys.id} className="flex min-w-0 items-center gap-0.5">
              ...
              <button
                className={`truncate rounded px-1 py-0.5 hover:bg-[#e3eaf2] ${
        ...
        <div className="flex items-center gap-0.5">
          ...
          <button
            className={`ss-toolbtn ${showMiniMap ? "bg-[#d3deea]" : ""}`}
      ...
      <div
        ...
        onDoubleClick={(e) => {
          // Drill into sub-systems by coordinate hit-test: the second click of a
          // double-click can land on the pane while React re-renders the
          // selection, so relying on onNodeDoubleClick alone is not enough.
          if (!system) return;
          ...
          if (hit?.subSystemId) store.getState().setActiveSystem(hit.subSystemId);
        }}
      >
        <ReactFlow
          nodes={nodes}
        ...
        >
          <Background variant={BackgroundVariant.Lines} gap={22} color="#eceff3" />
          {showMiniMap && (
            <MiniMap
              pannable
              zoomable
              ...
              bgColor="#f2f4f8"
              maskColor="rgba(47, 111, 179, 0.09)"
              nodeColor="#7e8ca0"
              nodeStrokeColor="#5b6472"
            />
          )}
        </ReactFlow>
