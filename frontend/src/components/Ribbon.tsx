  ...
  return (
    <button
      className={`flex h-[52px] w-[58px] flex-col items-center justify-center gap-0.5 rounded text-[11px] leading-tight
        ...
        hover:bg-[#e3eaf2] active:bg-[#d3deea] disabled:opacity-40 disabled:hover:bg-transparent`}
      onClick={onClick}
      disabled={disabled}
      title={title ?? label}
    >
...
function OpenProjectButton() {
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      />
      {open && (
        <div className="absolute left-0 top-[54px] z-50 min-w-[220px] rounded border border-[color:var(--ss-border)] bg-white py-1 shadow-lg">
          {items.length === 0 && (
...
export function Ribbon() {
  ...
  return (
    <div className="shrink-0 border-b border-[color:var(--ss-border)] bg-[color:var(--ss-chrome)]">
        ...
        {TABS.map((t) => (
          <button
            ...
            className={`rounded-t px-3 py-1 text-[12px] ${
              tab === t.id
                ? "border border-b-0 border-[color:var(--ss-border)] bg-white font-semibold text-[color:var(--ss-accent)]"
                : "text-[color:var(--ss-text)] hover:bg-[#e2e7ee]"
            }`}
            onClick={() => setTab(t.id)}
          >
            {t.label}
      ...
      <div className="flex h-[72px] items-stretch border-t border-[color:var(--ss-border)] bg-white px-1">
        {tab === "home" && <HomeTab />}
